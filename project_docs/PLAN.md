...
content lives under a different container. **Stage 12 (News) is struck** — checked 2026-09-09,
`tt_news` *is* the timeline and Stage 3 owns all of it; the only other store, pid 1045
`Bio (Kopie 1)`, is a hidden backup copy of the same sysfolder with nothing unique in it.
**Only Stages 13 (Misc, incl. container 1049) and 14 (reconciliation) remain.** Per-stage detail:
...
- [ ] News feed: in or out of scope for this redesign? **(Design question, still open — but
  note the migration side is now settled: there is NO legacy news content. `tt_news` is the
  timeline, Stage 12 struck 2026-09-09. A news feed would be new content, not migrated.)**
